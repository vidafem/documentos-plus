...
export default function DelegacionesFlagranciaModule() {
    ...
    const safeRows = ((data || []) as GenericRow[]).map((row) => {
      ...
      DELEGACIONES_HEADERS.forEach((header) => {
        ...
        if (header === "APELLIDOS_Y_NOMBRES_DE_LAS_PERSONAS_SOSPECHOSAS_QUE_SE_HA_EMITIDO_BOLETA_DE_CAPTURA") {
          ...
          dbKey = "APELLIDOS_Y_NOMBRES_DE_LOS_DETENIDOS_PRODUCTO_DEL_CUMPLIMIENTO_";
        }
  ...
  const guardarCampoVacio = async () => {
    ...
    if (columnToUpdate === "APELLIDOS_Y_NOMBRES_DE_LAS_PERSONAS_SOSPECHOSAS_QUE_SE_HA_EMITIDO_BOLETA_DE_CAPTURA") {
      ...
      columnToUpdate = "APELLIDOS_Y_NOMBRES_DE_LOS_DETENIDOS_PRODUCTO_DEL_CUMPLIMIENTO_";
    }

    const { error } = await supabase
      .from("DELEGACIONES")
      .update({ [columnToUpdate]: valueToSave })
      .eq("ORDEN", orden);
