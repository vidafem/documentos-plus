...
const mapRecordToFormState = (record: Record<string, unknown>): FormState => {
  ...
  let ifAnio = String(new Date().getFullYear());
  let ifSecuencial = "";

  if (ifRaw.startsWith("0901018") && ifRaw.length >= 15) {
    ifAnio = `20${ifRaw.slice(7, 9)}`;
    ifSecuencial = ifRaw.slice(9, 15);
  } else if (ifRaw.startsWith("901018") && ifRaw.length >= 14) {
    ifAnio = `20${ifRaw.slice(6, 8)}`;
    ifSecuencial = ifRaw.slice(8, 14);
  } else if (ifRaw.length >= 8) {
    const ifAnio2 = ifRaw.slice(6, 8);
    ifAnio = `20${ifAnio2}`;
    ifSecuencial = ifRaw.slice(8, 14);
  }
...
export default function FormDelegacionesDiarias({
  ...
  useEffect(() => {
    ...
    const hydrateFromRecord = async () => {
      ...
      if (initialRecord["caso_pj"] !== undefined && initialRecord["caso_pj"] !== null) {
        setCasoPj(!!initialRecord["caso_pj"]);
      } else if (editId) {
        const { data: deleRow } = await supabase
          ...
          .eq("ORDEN", String(editId))
          .maybeSingle();

        if (deleRow) {
          const hasDetenidos = !!(deleRow.NUMERO_DE_DETENIDOS_PRODUCTO_DE_LA_INVESTIGACION || deleRow.APELLIDOS_Y_NOMBRES_DE_LOS_DETENIDOS_PRODUCTO_DEL_CUMPLIMIENTO_);
          setCasoPj(hasDetenidos);
        } else {
          setCasoPj(false);
        }
      } else {
      ...
      const unidadSeleccionada = opcionesUnicas.includes(mapped.unidadEspecializadaFiscalia)
        ? mapped.unidadEspecializadaFiscalia
        : (opcionesUnicas[0] || "");
      ...
      setFormData((prev) => ({ ...prev, unidadEspecializadaFiscalia: unidadSeleccionada }));
    };
  ...
  }, [formData.ifAnio, formData.ifSecuencial, mode, editId]);

  // Consulta automática a Fiscalía al completar 15 dígitos
  useEffect(() => {
  ...
  const handleAutofillFiscalia = async () => {
    ...
    if (fiscaliaResult.delito) {
      await buscarDelitos(fiscaliaResult.delito);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "edit") {
      ...
      const resp = await supabase.from("FLAGRANCIA").insert([payload]).select("id");
      error = (resp.error as { message: string } | null);
      if (!error && resp.data?.[0]) {
    ...
    if (error && /UNIDAD_ESPECIALIZADA_DE_FISCALIA|invalid input syntax|integer|numeric|type/i.test(error.message)) {
      ...
      if (mode === "edit") {
        ...
        const reintento = await supabase.from("FLAGRANCIA").insert([payloadConUnidadNumerica]).select("id");
        error = (reintento.error as { message: string } | null);
        if (!error && reintento.data?.[0]) {
          newId = reintento.data[0].id;
        }
      }
    }

    let deleSuccessMessage = "";
    let deleErrorMessage = "";
    ...
    if (deleSuccessMessage) {
      ...
      setNotification({
        message: mode === "edit" ? "Registro actualizado en FLAGRANCIA" : "Registro guardado en FLAGRANCIA",
        type: "success",
      });
    }
    setPeritoSugerencias([]);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-[7.5rem_13.5rem_6.5rem_7rem_6rem_4.5rem_minmax(12rem,1fr)] gap-2 items-start">
        ...
        <div className="space-y-1">
          ...
          <div className={`flex items-center bg-white/5 border rounded-xl overflow-hidden ${
        ...
        <div className="space-y-1 flex flex-col justify-end h-full">
          <span className="text-[10px] font-bold text-white/40 uppercase">Caso PJ</span>
          <label className="relative inline-flex items-center cursor-pointer h-[38px] select-none">
            <input
              type="checkbox"
              checked={casoPj}
              onChange={(e) => setCasoPj(e.target.checked)}
            ...
            <span className="ml-2 text-xs font-bold text-white/70 uppercase">
              {casoPj ? "SÍ" : "NO"}
            </span>
          </label>
        </div>
