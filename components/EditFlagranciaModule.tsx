...
const toDateOnly = (val: unknown): Date | null => {
  if (!val) return null;
  const txt = String(val).trim().replace(/\//g, "-");
  if (!txt || /^null$/i.test(txt)) return null;
...
export default function EditFlagranciaModule() {
  ...
  const guardarCumplimiento = async () => {
    ...
    // Validar que la fecha de Extracto sea mayor a las fechas establecidas
    const exDateStr = `${cumplimientoForm.exAnio}-${cumplimientoForm.exMes}-${cumplimientoForm.exDia || "01"}`;
    const exDate = toDateOnly(exDateStr);
    if (!exDate) {
      setNotification({ message: "La fecha de Extracto (Fecha) no es válida.", type: "error" });
      return;
    }

    const fDelegacion = toDateOnly(cumplirItem.F_DELEGACION);
    const fRecepcion = toDateOnly(cumplirItem.F_RECEPCION);
    const fAgente = toDateOnly(cumplirItem.FECHA_DE_RECEPCION_POR_PARTE_AGENTE_INVESTIGADOR);

    if (fDelegacion && exDate.getTime() <= fDelegacion.getTime()) {
      setNotification({
        message: `La fecha de Extracto (${formatDateToDDMMYYYY(exDate)}) debe ser mayor a la fecha de la delegación (${formatDateToDDMMYYYY(fDelegacion)}).`,
        type: "error",
      });
      return;
    }

    if (fRecepcion && exDate.getTime() <= fRecepcion.getTime()) {
      setNotification({
        message: `La fecha de Extracto (${formatDateToDDMMYYYY(exDate)}) debe ser mayor a la fecha de recepción en la PJ (${formatDateToDDMMYYYY(fRecepcion)}).`,
        type: "error",
      });
      return;
    }

    if (fAgente && exDate.getTime() <= fAgente.getTime()) {
      setNotification({
        message: `La fecha de Extracto (${formatDateToDDMMYYYY(exDate)}) debe ser mayor a la fecha de recepción por el agente (${formatDateToDDMMYYYY(fAgente)}).`,
        type: "error",
      });
      return;
    }
