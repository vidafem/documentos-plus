...
export default function FormPartes() {
  ...
  const [ppUltimos10, setPpUltimos10] = useState(""); // Mantenemos el nombre del estado pero ahora acepta 12
  ...
  const obtenerUltimosDiasPorMesYAnio = async (targetAnio: string, targetMes: string) => {
    ...
    try {
      ...
      console.error("Error consultando últimos días del mes en partes_viejas:", err);
    }
  };

  // Inicializar año automáticamente desde el último registro de partes_viejas
  useEffect(() => {
    let active = true;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('partes_viejas').insert([registro]);
