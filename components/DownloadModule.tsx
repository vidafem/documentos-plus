  ...
  useEffect(() => {
    ...
    const loadYears = async () => {
      ...
      try {
        ...
        const maxVal = (((maxRes.data || []) as unknown) as GenericRow[])[0]?.["fecha_cierre"];
        const minVal = (((minRes.data || []) as unknown) as GenericRow[])[0]?.["fecha_cierre"];
        ...
        if (!active) return;
        setAniosCierreDisponibles([fallbackYear]);
      }
    };
...
<body>
  ...
  const descargarTodosPdfUnificado = async () => {
    ...
    try {
      ...
      const stamp = new Date().toISOString().slice(0, 10);
      mergedPdf.save(`DELEGACIONES_VIEJAS_PDF_${stamp}.pdf`);
