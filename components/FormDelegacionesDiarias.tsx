...
export default function FormDelegacionesDiarias() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!error && casoPj && newId) {
      ...
      if (deleError) {
        console.error("Error al guardar caso PJ en DELEGACIONES:", deleError);
      }
    }
    ...
    setFormData((prev) => {
      let nextAudicencia = "";
      if (prev.fAudicencia) {
        const parts = prev.fAudicencia.split("T")[0].split("-");
        if (parts.length >= 2) {
          nextAudicencia = `${parts[0]}-${parts[1]}-01T00:00`;
        }
      }
      return {
        ...initialState,
        ...
        fechaInfraccionMes: prev.fechaInfraccionMes,
  ...
  return (
    <>
      ...
      <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-3">
        <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-[9rem_12rem_10rem] gap-3 items-end">
          <div className="space-y-1">
